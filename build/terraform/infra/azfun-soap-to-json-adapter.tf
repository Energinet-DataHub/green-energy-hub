module "azfun_soaptojsonadapter" {
  ...
  name                                      = "azfun-soaptojsonadapter-${var.organisation}-${var.environment}"
...
module "azfun_soaptojsonadapter_plan" {
  ...
  name                = "asp-soaptojsonadapter-${var.organisation}-${var.environment}"
  resource_group_name = data.azurerm_resource_group.greenenergyhub.name
  location            = data.azurerm_resource_group.greenenergyhub.location
  kind                = "FunctionApp"
  sku                 = {
...
module "azfun_soaptojsonadapter_stor" {
  ...
  name                      = "stor${random_string.soaptojsonadapter.result}${var.organisation}${lower(var.environment)}"
  resource_group_name       = data.azurerm_resource_group.greenenergyhub.name
  location                  = data.azurerm_resource_group.greenenergyhub.location
