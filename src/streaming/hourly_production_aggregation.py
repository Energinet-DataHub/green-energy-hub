...
# %%
p = configargparse.ArgParser(prog='hourly_production_aggregation.py', description='Green Energy Hub Hourly Production Aggregation',
                             default_config_files=['configuration/run_args_hourly_production.conf'],
...
p.add('--output-path', type=str, required=False, default="delta/hourly-production-grid-area/",
...
# %%
from geh_stream.aggregation_utils.aggregators import HourlyProductionAggregator

# Perform aggregation calculation
aggregated_df = HourlyProductionAggregator.aggregate(valid_time_period_df)
